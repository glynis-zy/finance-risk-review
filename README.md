...
| 异步 | 进程内 asyncio 队列 + 前端轮询（无 Celery/Redis） |
| LLM | DeepSeek（适配层，厂商可换）——合同字段提取 / 对话 NLU / 报告润色 |
| OCR | 百度云（发票专用 + 通用）——三级 fallback：AUTO→预制案例→失败 |
...
`admin` 登录后左侧出现"系统管理"：用户管理（建用户/分配角色/启停）、角色权限（勾选权限码）、系统参数（风险升级阈值、OCR 模式 `auto/preset` 等，改完下一次分析生效）。设计原则与设计模式见 `docs/DESIGN.md`。
...
配置后：附件走真实 OCR（发票专用识别 / 通用识别），合同走"OCR 全文 → LLM 结构化提取（Pydantic 校验）"，报告由 LLM 润色。未配置则命中 `demo/preset_parse/` 预制结果，演示链路依然确定性可跑。
