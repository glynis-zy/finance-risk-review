...
### 1.8 可测试性
规则引擎是**确定性规则**（无随机、无 LLM 调用，可读 DB/配置），配合 `pytest` 可单测；依赖全部走 `Depends` 注入（`get_db`/`get_current_user`），冒烟测试用 `TestClient` 全链路断言。
...
A: ①新增规则只注册一个函数，不碰调用方（开闭）；②每条规则可独立启停/配阈值（策略）；③确定性函数可单测（pytest 用例）。规模再大可以拆成独立 worker。
